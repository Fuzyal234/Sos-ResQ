...
class ContactController {
  async create(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      const sosUser = await sosUserService.getSosUserByUserId(sos_user_id);
      if (!sosUser) {
        return reply.status(404).send(errorResponse('User not found', 404));
      }
      const user_phone = sosUser.phone_number;
      const hasSamePhoneNumber = contactsWithUserId.some(
        contact => contact.phone === user_phone,
      );
      if (hasSamePhoneNumber) {
        return reply
          ...
          .send(
            errorResponse(
              'You cannot add your own phone number as a contact',
              400,
            ),
          );
      }
      const newContacts = await contactService.createContacts(
        contactsWithUserId,
      );
      return reply
        .status(201)
        .send(
          successResponse('Contacts created successfully!', newContacts, 201),
        );
    } catch (error) {
  ...
  async index(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
      ...
      return reply
        .status(200)
        .send(successResponse('Contacts fetched successfully!', contacts, 200));
    } catch (error) {
      ...
      if (error instanceof Error && error.name === 'SequelizeDatabaseError') {
        return reply
          .status(500)
          .send(errorResponse('Database error occurred', 500));
      }
