...
const socketPlugin: FastifyPluginAsync<FastifySocketioOptions> = fp(async function (
  fastify,
  opts: FastifySocketioOptions,
) {
  ...
  fastify.io.on('connection', async socket => {
    ...
    if (token) {
      ...
      if (socket.data.role === 'sos_user') {
        ...
        console.log('saving the user socket in redis', socket.data.user, socket.id);
        redisService.setUserSocket(socket.data.user, socket.id);
        const room = await redisService.getUserRoom(socket.data.user);
        console.log('room :>> ', room);
        if (room) {
          socket.join(room);
          socket.emit('connected_to_agent', room);
          socket.to(room).emit('connected_to_agent', room);
        }
      }
      socketService.registerSocketEvents(socket, fastify.io);
      console.log('socket.data :>> ', socket.data);
      if (socket.data.role === 'agent') {
        console.log('saving the agent socket in redis', socket.data.user, socket.id);
