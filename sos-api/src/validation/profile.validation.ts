export const validateProfileUpdateFields = (body: any) => {
    ...
    const requiredFields = ['full_name', 'gender', 'date_of_birth', 'address', 'avatar'];
    ...
    if (body.gender && !["male", "female", "other"].includes(body.gender?.value)) {
    ...
    if (body.address && body.address?.value?.length > 100) {
      errors.push({ field: 'address', message: 'Address cannot be more than 100 characters' });
    }
